...
function serialize(rules: OrderDiscountRules): string {
  return JSON.stringify({
    ...
    maxQtyPerLine: {
      ...
      countBy: rules.maxQtyPerLine.countBy,
    },
  });
}
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-7">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={rules.maxQtyPerLine.countBy}
                onValueChange={(v) =>
                  setMaxQty({ countBy: v as MaxQtyCountBy })
                }
              >
                <SelectTrigger>
                  <SelectValue />
                ...
                <SelectContent>
                  <SelectItem value="VARIANT">Each order line</SelectItem>
                  <SelectItem value="PRODUCT">
                    Product (all lines combined)
                  </SelectItem>
                  <SelectItem value="CATEGORY">
                    Category (all lines combined)
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          <p className="text-xs text-muted-foreground pl-7">
            {rules.maxQtyPerLine.countBy === "VARIANT"
              ? `Each order line is discounted on its first ${rules.maxQtyPerLine.value} unit(s).`
              : rules.maxQtyPerLine.countBy === "PRODUCT"
                ? `The first ${rules.maxQtyPerLine.value} unit(s) of a product are discounted across every line of that product in the order.`
                : `The first ${rules.maxQtyPerLine.value} unit(s) within a category are discounted across every line in that category.`}
          </p>
        </CardContent>
      </Card>
    </div>
