...
function getColumnIndexByHeader(headerRow: ExcelJS.Row | undefined, headerName: string) {
  try {
    let colIndex = null;
...
export function ProductUploadExcel() {
  ...
  const validateExcelHeaders = async (file: File): Promise<string[]> => {
    ...
    try {
      ...
      errors.push("Failed to read Excel file. Please ensure it's a valid Excel format.");
    }
    ...
    try {
      ...
      sheet?.eachRow((row, index) => {
        if (index > 1) {
          ...
          const descriptionColumn = getColumnIndexByHeader(headerRow, "Name");
          const vendorNameColumn = getColumnIndexByHeader(headerRow, "Vendor Name");
          const costColumn = getColumnIndexByHeader(headerRow, "Cost");
          const rspColumn = getColumnIndexByHeader(headerRow, "Price");
          const estimationGPColumn = getColumnIndexByHeader(headerRow, "Estimation GP");
          const categoryColumn = getColumnIndexByHeader(headerRow, "Product Group");
          const stocksColumn = getColumnIndexByHeader(headerRow, "Stocks");

          const input: ExcelRow = {
            barcode: barcodeColumn ? String(row.getCell(barcodeColumn).value || "") : "",
            description: descriptionColumn ? String(row.getCell(descriptionColumn).value || "") : "",
            vendorName: vendorNameColumn ? String(row.getCell(vendorNameColumn).value || "") : "",
            cost: costColumn ? Number(row.getCell(costColumn).value || 0) : 0,
            rsp: rspColumn ? Number(row.getCell(rspColumn).value || 0) : 0,
            estimationGP: estimationGPColumn ? Number(row.getCell(estimationGPColumn).value || 0) : 0,
            category: categoryColumn ? String(row.getCell(categoryColumn).value || "") : "",
            stocks: stocksColumn ? Number(row.getCell(stocksColumn).value || 0) : 0,
          };
      ...
      if (result?.success) {
        ...
        toast.success(`Excel uploaded successfully! ${processedRows} products added.`);
      ...
      toast.error("Failed to upload Excel file. Please check the format and try again.");
    }
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="font-medium">
                      {uploadProgress.phase === 'complete' ? 'Upload Complete!' : 
                       uploadProgress.phase === 'error' ? 'Upload Failed' : 'Processing...'}
                    </h3>
                    ...
                    {(uploadProgress.phase === 'parsing' || uploadProgress.phase === 'uploading') && (
                  ...
                  {uploadProgress.totalRows && (
                    <div className="text-xs text-gray-500">
                      {uploadProgress.processedRows || 0} of {uploadProgress.totalRows} rows processed
                    </div>
                  )}
          ...
          <Card>
            ...
            <CardContent className="pt-0 space-y-4">
              <div>
                <h4 className="text-xs font-medium text-red-600 mb-2">Required Headers:</h4>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {REQUIRED_HEADERS.map((header) => (
                    <div key={header} className="flex items-center gap-2 p-2 bg-red-50 border border-red-200 rounded">
              ...
              <div>
                <h4 className="text-xs font-medium text-gray-600 mb-2">Optional Headers:</h4>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {OPTIONAL_HEADERS.map((header) => (
                    <div key={header} className="flex items-center gap-2 p-2 bg-gray-50 border border-gray-200 rounded">
