"use client";
import { useUploadExcelUser } from "@/app/hooks/use-upload-excel-product";
...
import { UserExcelRow } from "@/classes/upload-from-excel";
...
export const uploadUserDialog = createDialog(
  ({ close }) => {
    const { trigger, isMutating } = useUploadExcelUser();
    ...
    const validateExcelHeaders = async (file: File): Promise<string[]> => {
      ...
      try {
        ...
        headerRow.eachCell((cell) => {
          if (cell.value) foundHeaders.push(String(cell.value));
        });
    ...
    const handleFileUpload = useCallback(async () => {
      ...
      try {
        ...
        sheet?.eachRow((row, index) => {
          if (index > 1) {
            const fullNameCol = getColumnIndexByHeader(headerRow, "Full Name");
            const usernameCol = getColumnIndexByHeader(headerRow, "Username");
            const phoneCol = getColumnIndexByHeader(headerRow, "Phone Number");
            const positionCol = getColumnIndexByHeader(headerRow, "Position");
            const profileCol = getColumnIndexByHeader(headerRow, "Profile");

            dataInput.push({
              fullName: fullNameCol
                ? String(row.getCell(fullNameCol).value ?? "")
                : "",
              username: usernameCol
                ? String(row.getCell(usernameCol).value ?? "")
                : "",
              phoneNumber: phoneCol
                ? String(row.getCell(phoneCol).value ?? "")
                : "",
              position: positionCol
                ? String(row.getCell(positionCol).value ?? "")
                : "",
              profile: profileCol
                ? String(row.getCell(profileCol).value ?? "")
                : "",
            });
          }
        });
        ...
        setUploadProgress({
          phase: "uploading",
          progress: 50,
          message: "Uploading users to server...",
          totalRows,
          processedRows: dataInput.length,
        });
        ...
        if (result?.success) {
          setUploadProgress({
            phase: "complete",
            progress: 100,
            message: `Successfully uploaded ${dataInput.length} users!`,
            totalRows,
            processedRows: dataInput.length,
          });
          toast.success(
            `Excel uploaded successfully! ${dataInput.length} users added.`,
          );
          setTimeout(() => {
            close(null);
          }, 2000);
    ...
    }, [selectedFile, trigger, close]);

    const downloadTemplate = useCallback(() => {
      const workbook = new ExcelJS.Workbook();
      const sheet = workbook.addWorksheet("Users");
      sheet.addRow(REQUIRED_HEADERS);
      sheet.addRow(["John Doe", "johndoe", "012-345-6789", "Cashier", "staff"]);
      const headerRow = sheet.getRow(1);
    ...
    return (
      <>
        <DialogHeader>
          ...
          <DialogDescription>
            Upload multiple users at once using an Excel file. Download the
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    >
                      ...
                      {isMutating ? "Uploading..." : "Upload Users"}
                    </Button>
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-xs">
                Required column headers for your Excel file:
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-0">
